...
// defined in war_emblem.c
// re-typedeffed here because its header file is not accessible to this component
typedef void* dspapp_handle_t;
void DspApp_processCmd(dspapp_handle_t, const char*);
extern dspapp_handle_t g_dsp_handle;
...
static esp_err_t pcm1681_write_reg(uint8_t reg_add, uint8_t data)
{
    ...
    do
    {
        result = i2c_bus_write_bytes(i2c_handler, PCM1681_ADDR, &reg_add, sizeof(reg_add), &data, sizeof(data));
        if (result != ESP_OK)
        {
...
{
    return i2c_bus_read_bytes(i2c_handler, PCM1681_ADDR, &reg_add, sizeof(reg_add), p_data, 1);
}
...
static esp_err_t i2c_init()
{
    ...
    i2c_handler = i2c_bus_create(I2C_NUM_0, &pcm1681_i2c_cfg);
    if (!i2c_handler)
    {
...
{
    return i2c_bus_delete(i2c_handler);
}
...
esp_err_t pcm1681_set_volume(int volume)
{
    ...
    // therefor attenuation is between 0 and -100
    ...
    return result;
#else
    // The esp-adf Bluetooth a2dp stream sends audio HAL set volume when it receives a Bluetooth AVRCP absolute volume
    // message We just need to set our DSP volume since we're not using the PCM1681's volume registers.
    char vol[30];
    snprintf(vol, 30, "Volume setUserValue %d", volume);
    DspApp_processCmd(g_dsp_handle, vol);
    return ESP_OK;
#endif
}
